...
const makeColumnClasses = ({ gapless, align, justify }) => {
   ...
   */
  if (gapless) {
    classesArray.push(styles[gridModifiers.gapless]);
  }
  if (align) {
    classesArray.push(styles[alignTypes[align]]);
  }
  if (justify) {
    classesArray.push(styles[justifyTypes[justify]]);
  }
...
export default function Grid(props) {
   ...
   */
  const properties = {
    className: classNames(styles[component], gridModifierClasses, props.className),
  };
